...
Tasarım sohbeti paralel ilerlerken kodlama hattı STATUS.md sırasını izler. #34 öneri p95 benchmarkı ve kişisel veri içermeyen kaba çalışma zamanı süreleriyle main'dedir. #35 bilinen yayın engellerini otomatik denetlenen bir kapıya ve geri dönüş runbook'una bağlar. Gerçek bulut restore provası, Sentry development projesi/source map kanıtı ve canlı analitik sağlayıcısı ayrı açık kapılardır. Kabul kriterleri STATUS.md, PERFORMANCE_RUNBOOK.md, RELEASE_RUNBOOK.md, FIREBASE_RUNBOOK.md, EVENT_OPERATIONS_RUNBOOK.md, OBSERVABILITY_RUNBOOK.md ve ANALYTICS_SPEC.md içindedir.
