...
async function documents(name: string, cityScoped: boolean): Promise<unknown[]> {
  ...
  return (await query.get()).docs.map(item => item.data()).sort((a, b) => String(a.id).localeCompare(String(b.id)));
}
...
async function main(): Promise<void> {
  ...
  for (const [name, expected, actual] of comparisons) {
    ...
    if (JSON.stringify(canonical(expectedCitySlice)) !== JSON.stringify(canonical(actualCitySlice))) {
      throw new Error(`${name}: local/Firestore document parity failed.`);
    }
  }
