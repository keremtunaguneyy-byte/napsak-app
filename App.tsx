  ...
  return <SafeAreaView edges={['top', 'right', 'bottom', 'left']} style={s.safe}>
    ...
    <ScrollView ref={scrollRef} contentContainerStyle={[s.page, width >= 700 && !isGuideArticle && s.pageWide, step === 'guides' && guideView === 'landing' && s.guideLandingPage, isGuideArticle && s.guideArticlePage]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} scrollEventThrottle={32} onScroll={event => {
      ...
      {step === 'interest' && <View>
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Mod seçimine geri dön" onPress={() => setStep('mood')} style={s.backHit}><Text style={s.back}>← Modumu değiştir</Text></TouchableOpacity>
      </View>}
      {step === 'budget' && <View>
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="İlgi seçimine geri dön" onPress={() => setStep('interest')} style={s.backHit}><Text style={s.back}>← İlgilerimi değiştir</Text></TouchableOpacity>
      </View>}
      {step === 'group' && <View>
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Bütçe seçimine geri dön" onPress={() => setStep('budget')} style={s.backHit}><Text style={s.back}>← Bütçemi değiştir</Text></TouchableOpacity>
      </View>}
      {step === 'duration' && <View>
      ...
      {step === 'saved' && <View>
        ...
        {!savedEntries.length && <View style={s.empty}><Text accessible={false} style={s.emptyIcon}>♡</Text><Text accessibilityRole="header" style={s.emptyTitle}>Henüz bir şey kaydetmedin</Text><Text style={s.emptyText}>Önerilerde veya Ankara 101’de “Kaydet”e dokunduğunda burada görünür.</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Ana sayfaya dön" style={s.emptyAction} onPress={() => setStep('results')}><Text style={s.emptyActionText}>Ana sayfaya dön</Text></TouchableOpacity></View>}
...
function Lead({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle: string }) { return <><Text style={s.eyebrow}>{eyebrow}</Text><Text accessibilityRole="header" style={s.title}>{title}</Text><Text style={s.subtitle}>{subtitle}</Text></>; }
  ...
  return <View>
    <ImageBackground accessible={false} source={ANKARA_CASTLE_HERO} resizeMode="cover" style={s.classicsHero}>
      <LinearGradient colors={['transparent', 'rgba(10,10,8,.12)', 'rgba(10,10,8,.92)']} locations={[0, .45, 1]} style={s.heroGradient}>
        <Text accessibilityRole="header" style={s.classicsHeroTitle}>ANKARA KLASİKLERİ</Text>
...
function ClassicChapter({ guide, index, saved, onSave, onOpenSource, onLayout }: { guide: Guide; index: number; saved: boolean; onSave: () => void; onOpenSource: () => void; onLayout: (y: number) => void }) {
  ...
  return <View onLayout={event => onLayout(event.nativeEvent.layout.y)} style={s.classicChapter}>
    ...
    <View style={s.classicActions}><TouchableOpacity accessibilityRole="button" accessibilityLabel={saved ? `${displayTitle} bölümünü kayıttan çıkar` : `${displayTitle} bölümünü kaydet`} accessibilityState={{ selected: saved }} onPress={onSave} style={s.classicActionHit}><Text style={s.classicAction}>{saved ? '♥ Kaydedildi' : '♡ Bölümü kaydet'}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="link" accessibilityLabel={`${displayTitle} kaynaklarını aç`} onPress={onOpenSource} style={s.classicActionHit}><Text style={s.classicSource}>Kaynaklar ↗</Text></TouchableOpacity></View>
  </View>;
}
...
function InsiderGuide({ saved, onBack, onSave, onOpenMap }: { saved: boolean; onBack: () => void; onSave: () => void; onOpenMap: () => void }) {
  ...
  return <View>
    ...
    <View style={s.insiderPaper}>
      ...
      <Text accessibilityRole="header" style={s.insiderTitle}>{route.title}</Text>
      ...
      <View style={s.insiderFeatureRow}>
        ...
        <View style={s.insiderAside}><Image accessible={false} source={TUNALI_CAFE_DETAIL} resizeMode="cover" style={s.insiderDetailImage} /><View style={s.friendNote}><View style={s.friendTape} /><Text style={s.friendNoteTitle}>ARKADAŞ NOTU</Text><Text style={s.friendNoteText}>{route.friendNote}</Text><Text accessible={false} style={s.friendHeart}>♡</Text></View></View>
      </View>
      ...
      <View style={s.insiderActions}><TouchableOpacity accessibilityRole="link" accessibilityLabel={`${route.title} rotasını haritada aç`} onPress={onOpenMap} style={s.insiderMapButton}><Text style={s.insiderMapButtonText}>⌖  Rotayı haritada aç</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel={saved ? `${route.title} rotasını kayıttan çıkar` : `${route.title} rotasını kaydet`} accessibilityState={{ selected: saved }} onPress={onSave} style={s.insiderSaveButton}><Text style={s.insiderSaveButtonText}>{saved ? '♥  Kaydedildi' : '♡  Kaydet'}</Text></TouchableOpacity></View>
      <TouchableOpacity accessibilityRole="button" accessibilityLabel="Ankara 101 seçimine dön" onPress={onBack} style={s.insiderBack}><Text style={s.insiderBackText}>← Ankara 101 seçimine dön</Text></TouchableOpacity>
    </View>
  </View>;
}
...
function RecommendationCard({ item, rank, saved, onSave, onDismiss, onOpenPlaceDetails, onOpenPlaceMaps, onOpenPlaceSource, onOpenIdea, onOpenEvent, onOpenExperienceMap, onOpenExperienceSource }: { item: RecommendationItem; rank: number; saved: boolean; onSave: () => void; onDismiss: () => void; onOpenPlaceDetails: (place: Place) => void; onOpenPlaceMaps: (place: Place) => void; onOpenPlaceSource: (place: Place) => void; onOpenIdea: (idea: Idea) => void; onOpenEvent: (event: Event) => void; onOpenExperienceMap: (experience: Experience) => void; onOpenExperienceSource: (experience: Experience) => void }) {
  ...
  return <View style={s.result}>
    ...
    <Text accessibilityRole="header" style={s.resultName}>{title}</Text>
...
const s = StyleSheet.create({
  ...
  header: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', alignItems: 'center', marginBottom: 44 }, headerActions: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 14 }, headerHit: { minHeight: 44, justifyContent: 'center' }, logo: { color: c.ink, fontFamily: 'SourceSans3_800ExtraBold', fontSize: 29, letterSpacing: -1.2 }, savedLink: { color: c.lime, fontSize: 12, fontWeight: '800', paddingVertical: 10 }, counter: { color: '#C4C1B8', fontSize: 12, fontWeight: '700', letterSpacing: 1.5 },
  ...
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 11 }, chip: { minHeight: 44, borderWidth: 1, borderColor: c.line, backgroundColor: c.card, borderRadius: 99, paddingVertical: 14, paddingHorizontal: 17 }, chipText: { color: c.ink, fontSize: 15, fontWeight: '700' }, backHit: { minHeight: 44, justifyContent: 'center', marginTop: 11 }, back: { color: c.muted, textAlign: 'center', fontWeight: '700' },
  ...
  filterRow: { gap: 8, paddingBottom: 18 }, filterChip: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: c.line, backgroundColor: c.card, borderRadius: 99, paddingVertical: 10, paddingHorizontal: 16 }, filterChipSelected: { backgroundColor: c.lime, borderColor: c.lime }, filterText: { color: c.ink, fontSize: 13, fontWeight: '800' }, filterTextSelected: { color: '#14160E' },
  ...
  undoBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#202518', borderWidth: 1, borderColor: '#56603A', borderRadius: 16, padding: 14, marginBottom: 14 }, undoText: { color: c.ink, fontSize: 13, fontWeight: '800' }, undoAction: { minHeight: 44, minWidth: 44, justifyContent: 'center', alignItems: 'flex-end' },
  ...
  empty: { alignItems: 'center', backgroundColor: c.card, borderWidth: 1, borderColor: c.line, borderRadius: 22, padding: 28, marginBottom: 18 }, emptyIcon: { color: c.lime, fontSize: 38, fontWeight: '900' }, emptyTitle: { color: c.ink, fontSize: 19, fontWeight: '900', marginTop: 14, textAlign: 'center' }, emptyText: { color: c.muted, fontSize: 14, lineHeight: 20, marginTop: 8, textAlign: 'center' }, emptyAction: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: c.lime, borderRadius: 14, paddingHorizontal: 18, paddingVertical: 12, marginTop: 20 }, emptyActionText: { color: c.lime, fontSize: 13, fontWeight: '900' }, removeAction: { color: '#FF9A8D', fontWeight: '800', fontSize: 13 },
